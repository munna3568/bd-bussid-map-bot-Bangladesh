...
FREE_CHANNEL_ID = -1004392467475 # BD ETS2 LOVER ANDROID
...
BKASH = "01832533534"
MY_ID = 7304092520 # এখানে @RawDataBot থেকে রিয়েল ID বসাবা
...
def get_social_buttons():
    return [[InlineKeyboardButton("📺 YouTube Subscribe", url=YOUTUBE_URL)], [InlineKeyboardButton("📘 Facebook Page Follow", url=FB_URL)]]
...
async def check_both_channels(user_id, context):
    try:
        ...
        logging.error(f"Channel check error: {e}"); return False
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if base_key == 'free':
        ...
        keyboard += back_btn; text = "🆓 নিচ থেকে ১টি ফ্রি ম্যাপ নাও:\n\n📢 *খুব শীঘ্রই ফ্রি নতুন ম্যাপ এড করা হবে*" + footer
        ...
        if await check_both_channels(user_id, context):
            name, link = FREE_MAPS[base_key]; download_btn = [[InlineKeyboardButton(f"📥 {name} ডাউনলোড করুন", url=link)]] + get_social_buttons() + back_btn
...
async def screenshot_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_data:
        ...
        try:
            ...
            # Approve + Reject 2টা বাটন
            keyboard = [
                [InlineKeyboardButton("✅ Approve & Send", callback_data=f"a_{user_id}_{map_key}")],
                [InlineKeyboardButton("❌ Reject - ভুল স্ক্রিনশট", callback_data=f"r_{user_id}_{map_key}")]
            ]
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(approve_handler, pattern='^a_')) # ছোট করলাম
    app.add_handler(CallbackQueryHandler(reject_handler, pattern='^r_')) # নতুন
    app.add_handler(MessageHandler(filters.PHOTO | filters.Document.ALL, screenshot_handler))
    print("BD Bussid Map Bot Running v6.4...")
