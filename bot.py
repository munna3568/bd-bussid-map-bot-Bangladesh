...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message: # /cancel থেকে আসলে user_data clear
        user_id = update.message.from_user.id
        if user_id in user_data: del user_data[user_id]
    await send_main_menu(update.message or update.callback_query.message)
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_data: del user_data[user_id] # মেনুতে আসলে state clear
...
async def screenshot_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_data:
        ...
        del user_data[user_id] # state clear
    return ConversationHandler.END

async def cancel_conv(update: Update, context: ContextTypes.DEFAULT_TYPE): # নতুন ফাংশন
    user_id = update.message.from_user.id
    if user_id in user_data: del user_data[user_id]
...
def main():
    ...
    # fallbacks add করলাম - এটাই মেইন ফিক্স
    conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(map_handler, pattern='^(map|p)')],
        states={
            WAITING_SCREENSHOT: [
                MessageHandler(filters.PHOTO, screenshot_handler),
                CommandHandler("cancel", cancel_conv), # /cancel
                CallbackQueryHandler(menu_handler, pattern='^(free|paid|back)') # মেনু বাটন চাপলে state থেকে বের হবে
            ]
        },
    ...
    app.add_handler(CallbackQueryHandler(menu_handler, pattern='^(free|paid|back)'))
    app.add_handler(CallbackQueryHandler(approve_handler, pattern='^approve_'))
    app.add_handler(conv)

    print("BD Bussid Map Bot Running v4.3...")
