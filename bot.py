...
async def map_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if base_key in FREE_MAPS:
        if await check_channel_member(user_id, context):
            ...
            download_btn = [[InlineKeyboardButton(f"📥 {name} ডাউনলোড করুন", url=link)]] + back_btn # লিংক বাটন
...
async def approve_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    footer = f"\n\n💳 পেমেন্ট: `{BKASH}`\n📢 চ্যানেল: {CHANNEL_USERNAME}"
    download_btn = [[InlineKeyboardButton(f"📥 {name} ডাউনলোড করুন", url=link)]] # পেইড এর জন্যও বাটন
...
def main():
    ...
    print("BD Bussid Map Bot Running v4.1...")
