...
CHANNEL_LINK = "@bdbussidlover" # t.me/ বাদ দিয়ে @ দিলাম
BKASH = "01832533534"
MY_ID = 7304092520
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = "🚌 **BD Bussid Map Bot**\n\nনিচ থেকে ম্যাপ সিলেক্ট করো:\n\n"
    text += f"💳 পেমেন্ট করুন: bKash/Nagad/Rocket: `{BKASH}`\n"
    text += f"📢 আপডেট চ্যানেল: {CHANNEL_LINK}"
    await update.message.reply_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown', disable_web_page_preview=True) # Preview বন্ধ
...
async def map_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data in FREE_MAPS:
        ...
        if "DRIVE_LINK" in link:
            await query.edit_message_text("⚠️ এই ম্যাপের লিংক এখনো এড করা হয়নি।"); return ConversationHandler.END
        text = f"💰 **{name}**\n\nপেমেন্ট: **40 টাকা**\nবিকাশ/নগদ/রকেট: `{BKASH}`\n\nটাকা পাঠিয়ে স্কিনশট এই চ্যাটে পাঠাও।" + footer
        await query.edit_message_text(text, parse_mode='Markdown', disable_web_page_preview=True); return WAITING_SCREENSHOT
