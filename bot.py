...
async def unknown_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    if user_id in user_data: await update.message.reply_text("⚠️ আগে স্ক্রিনশট পাঠান। অথবা `Back to Menu` চেপে বের হয়ে আসুন।", parse_mode='Markdown'); return
    keyboard = [[InlineKeyboardButton("🔄 /start - বট চালু করুন", callback_data=get_callback_data('back'))]]
    await update.message.reply_text("❌ ভাই বুঝলাম না।\n\nইংরেজিতে লেখা `Menu` বাটনে চাপুন অথবা `/start` লিখে পাঠান।", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
