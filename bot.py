...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [[InlineKeyboardButton("🆓 ফ্রি ম্যাপ ৩টি", callback_data='free')], [InlineKeyboardButton("💎 পেইড ম্যাপ 18টি - 40 টাকা", callback_data='paid')]]
...
async def map_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data in FREE_MAPS:
        if await check_channel_member(user_id, context):
            name, link = FREE_MAPS[data]; await query.edit_message_text(f"✅ **{name}**\n\nডাউনলোড লিংক: `{link}`", parse_mode='Markdown')
        else:
        ...
        if "DRIVE_LINK" in link: await query.edit_message_text("⚠️ এই ম্যাপের লিংক এখনো এড করা হয়নি।"); return ConversationHandler.END
        text = f"💰 **{name}**\n\nপেমেন্ট: **40 টাকা**\nবিকাশ/নগদ/রকেট: `{BKASH}`\n\nটাকা পাঠিয়ে স্কিনশট এই চ্যাটে পাঠাও।"
        await query.edit_message_text(text, parse_mode='Markdown'); return WAITING_SCREENSHOT
