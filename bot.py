...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'free':
        keyboard = [[InlineKeyboardButton(name[0], callback_data=key)] for key, name in FREE_MAPS.items()] + back_btn
        ...
        keyboard = [[InlineKeyboardButton(f"{name[0]} - 40৳", callback_data=key)] for key, name in PAID_MAPS.items()] + back_btn
...
async def map_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data in FREE_MAPS:
        if await check_channel_member(user_id, context):
            ...
            keyboard = [[InlineKeyboardButton("🔄 জয়েন করে চেক করুন", callback_data=data)]] + back_btn
...
def main():
    ...
    print("BD Bussid Map Bot Running v3.0...")
