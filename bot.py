...
def save_user(user_id):
    ...
    try:
        c.execute("INSERT INTO users (user_id, first_join) VALUES (?,?)", (user_id, today))
        conn.commit()
...
def get_stats():
    ...
    c.execute("SELECT COUNT(*) FROM users WHERE first_join LIKE?", (f"{today}%",))
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.from_user.id!= MY_ID:
        await update.message.reply_text("⚠️ এই কমান্ড শুধু এডমিনের জন্য")
        return
...
async def send_main_menu(target):
    ...
    footer = f"\n\n💳 সেন্ড মানি: `{BKASH}`\n📢 চ্যানেল: {CHANNEL_USERNAME}" # এখানে চেঞ্জ
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    footer = f"\n\n💳 সেন্ড মানি: `{BKASH}`\n📢 চ্যানেল: {CHANNEL_USERNAME}"; back_btn = [[InlineKeyboardButton("⬅️ Back to Menu", callback_data=get_callback_data('back'))]] # এখানে চেঞ্জ
    if base_key == 'free':
...
def main():
    ...
    print("BD Bussid Map Bot Running v7.6...")
